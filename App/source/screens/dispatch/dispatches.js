...
const MasterScreen = ({navigation}) => {
    ...
    const toggleTabs = async (status) => {
        setLoading(true)
        if (status != dispatched) {
            setDispatched(!dispatched)
            await filterData(!dispatched, dispatchesData)
        }
    }
    ...
    const onPressDispatch = async (dispatchesId, assigned, time) => {
        if (assigned) {
            // await navigation.navigate('AssignPrice', {clientId: null, dispatchDate: time})
            await navigation.navigate('AssignPrice', {clientId: null, dispatchesId: dispatchesId, dispatchDate: time})
        } else {
            await navigation.navigate('AssignItems', {dispatchesId: dispatchesId, authToken: authToken, date: time})
        }
    }
        ...
        return (
            <TouchableOpacity key={item.id} style={styles.item} onPress={() => onPressDispatch(id, assigned,time)}>
                ...
                <TouchableOpacity onPress={() => onPressDispatch(id, assigned, time)} style={styles.boxToSeller}>
                    <Text style={styles.dispatcher}> {assigned ? 'Assign Price' : 'Assign boxes to seller'}</Text>
                </TouchableOpacity>
            </TouchableOpacity>
        )
    };
        ...
        <>
            <SafeAreaView style={styles.container}>
                <View style={{flexDirection: 'row', margin: 5}}>
                    <TouchableOpacity onPress={toggleTabs} style={[styles.tabBtn, {backgroundColor: !dispatched ? 'black' : 'white'}]}>
                        <Text style={{color: dispatched ? 'black' : 'white'}}>Available</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={toggleTabs} style={[styles.tabBtn, {backgroundColor: dispatched ? 'black' : 'white'}]}>
                        <Text style={{color: !dispatched ? 'black' : 'white'}}>Dispatched</Text>
                    </TouchableOpacity>
                </View>
